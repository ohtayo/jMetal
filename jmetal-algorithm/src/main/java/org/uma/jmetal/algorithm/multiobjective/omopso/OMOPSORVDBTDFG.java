...
public class OMOPSORVDBTDFG extends OMOPSOWithSizeLimitedArchive {
  ...
  /** Constructor */
  public OMOPSORVDBTDFG(DoubleProblem problem, SolutionListEvaluator<DoubleSolution> evaluator,
  ...
  protected void updateVelocity(List<DoubleSolution> swarm)  {
    ...
    for (int i = 0; i < swarmSize; i++) {
      ...
      // (2)[DFG] 対象particleとarchiveが同一ランクであれば，同一ランクarchiveから近い10個体のうちランダムでvelocityを足し合わせる
        ...
        updateVelocityUsingSameRankArchivePosition(W, C1, C2, i, particle, sameRankArchive);  // (b-2-II)の手法を用いる
      }
  ...
  protected void updateVelocityUsingDominanceArchive(
      double W, double C1, double C2, int i, ParticleSwarmSolution particle, List<ParticleSwarmSolution> dominanceArchive, DoubleSolution bestParticle
  ){
    double r1, r2;
  ...
  protected void updateVelocityUsingSameRankArchivePosition(
      double W, double C1, double C2, int i, ParticleSwarmSolution particle, List<ParticleSwarmSolution> sameRankArchive
  ) {
    double r1, r2;
  ...
  protected void updateVelocityUsingNoGlobalBest(
      double W, double C1, double C2, int i, ParticleSwarmSolution particle, DoubleSolution bestParticle )
  {
    double r1, r2;
    for (int var = 0; var < particle.getNumberOfVariables(); var++) {
      r1 = randomGenerator.nextDouble();
