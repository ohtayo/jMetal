...
import org.uma.jmetal.algorithm.multiobjective.omopso.DirectiveOMOPSO;
...
public class ParallelDirectionalOMOPSOWithSizeLimitedArchiveRunner extends AbstractAlgorithmRunner {
  /**
...
//    problem = (DoubleProblem) ProblemUtils.<DoubleSolution> loadProblem(problemName, (Object[]) problemArgs);
    ...
    OMOPSOBuilder builder = new OMOPSOBuilder(problem, evaluator)
            ...
            .setVariant(OMOPSOBuilder.OMOPSOVariant.DirectiveOMOPSO);
    algorithm = builder.build();

    // 初期値のファイル名の指定があれば初期値を設定
    if(!StringUtility.isNullOrEmpty(fileNameOfInitialSolutions)){
      List<DoubleSolution> initialSwarm = ((DirectiveOMOPSO) algorithm).createInitialSwarm();
      ...
      ((DirectiveOMOPSO) algorithm).setInitialSwarm(initialSwarm);
